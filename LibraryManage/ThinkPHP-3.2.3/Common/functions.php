...
function load($name, $baseUrl = '', $ext = '.php')
{
    ...
    if (empty($baseUrl)) {
        if (0 === strpos($name, '@/')) {
            //加载当前模块函数库
            $baseUrl = MODULE_PATH . 'Base/';
            $name    = substr($name, 2);
        } else {
            //加载其他模块函数库
            $array   = explode('/', $name);
            $baseUrl = APP_PATH . array_shift($array) . '/Base/';
            $name    = implode('/', $array);
        }
    }
    if (substr($baseUrl, -1) != '/') {
...
function D($name = '', $layer = '')
{
    ...
    if (class_exists($class)) {
        ...
        if (!C('APP_USE_NAMESPACE')) {
            import('Base/' . $layer . '/' . $class);
        } else {
            $class = '\\Base\\' . $layer . '\\' . $name . $layer;
        }
